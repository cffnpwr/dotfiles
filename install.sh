...
# Rename /etc files that nix-darwin manages to avoid activation conflicts
# nix-darwin will abort activation if it finds unrecognized content in managed /etc files
_prepare_etc() {
  if [ -f /etc/zshenv ] && [ ! -L /etc/zshenv ]; then
    gum log --level info "Renaming /etc/zshenv to /etc/zshenv.before-nix-darwin..."
    sudo mv /etc/zshenv /etc/zshenv.before-nix-darwin
    gum log --level info "Renamed /etc/zshenv successfully"
  fi

  # /etc/nix/nix.custom.conf is created by Determinate Nix installer but also
  # managed by nix-darwin's determinateNix module; remove it to avoid conflict
  if [ -f /etc/nix/nix.custom.conf ] && [ ! -L /etc/nix/nix.custom.conf ]; then
    gum log --level info "Removing /etc/nix/nix.custom.conf (will be regenerated by nix-darwin)..."
    sudo rm /etc/nix/nix.custom.conf
    gum log --level info "Removed /etc/nix/nix.custom.conf successfully"
  fi
}
...
_apply_dotfiles() {
  ...
  sudo NIX_CONFIG="${NIX_CONFIG:-}" $switch_cmd switch --impure --flake "${REPO_PATH}#${HOSTNAME}"
